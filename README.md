...
index cfe49ce45eeeebf21b76e18eaca7a6a164c80757..a76c1c636a9016e6b6a295367b7b87842d2af423 100644
--- a/README.md
+++ b/README.md
@@ -1,263 +1,27 @@
...
-EOF
-)
+# BLUE JEANS SCRIPT DOCTOR v1.2 (Streamlit MVP)
+
+## 실행 방법
+```bash
+python -m venv .venv
...
+- `OPENAI_API_KEY`
+- `GEMINI_API_KEY` (옵션)
+- `MODEL_NAME` (기본값 코드 내 제공)
+- `PROMPT_VERSION`
+- `MAX_PAGES`, `MAX_TOKENS`, `RETRY_LIMIT`
+- `ADMIN_MODE=1` 설정 시 detail에서 `results.zip` 다운로드 노출
+
+## 출력 정책
+- 사용자 기본 다운로드: `Blue.docx` 형식 보고서 1개
+- 내부 저장: run_id 폴더에 `meta.json`, `raw_analysis.json`, `charts/*.png`, `docx`
+
+## 예외 처리
+- PDF 추출 실패, LLM 실패/JSON 파싱 실패, 차트 생성 실패 시에도 앱이 종료되지 않도록 안전 기본값으로 진행
+- API Key 및 원문 전체 로그 저장은 피하고, 요약/해시만 메타 경고로 남김
